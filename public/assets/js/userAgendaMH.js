...
(function () {
  ...
  userMountSidebar("agenda");

  // Logout (sidebar e mobile)
  document.addEventListener("click", (e) => {
    const t = e.target;
    const btn = t?.closest?.("#btnLogoutTop, #btnLogoutUser");
    if (!btn) return;
    API.clearAuth();
  ...
  function openDetails(ev) {
    ...
    if (dCreator) dCreator.textContent = isOwner ? "Você" : memberName(ev.createdBy);

    const partNames = parts.map(memberName).filter(Boolean);
    if (dParticipants) {
      dParticipants.textContent = partNames.length ? partNames.join(", ") : "—";
    }

    // texto auxiliar (permissão/edição)
    const total = parts.length;
    dPerm.textContent = isOwner
      ? `Você criou este evento${total ? ` • ${total} participante(s)` : ""}`
      : `Somente o criador pode editar/excluir${total ? ` • ${total} participante(s)` : ""}`;
